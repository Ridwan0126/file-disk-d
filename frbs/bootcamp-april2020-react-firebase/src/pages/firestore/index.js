...
class FirestoreInput extends Component {
    ...
    fetchAllData = async () => {
        ...
        if (vehicleList.length > 0) this.setState({
    ...
    subscribeData = () => {
        ...
        this.subscribeVehicles = this.props.firebase.getUpdateFirestoreVehicle(vehicles => {
            ...
            vehicles.docChanges().forEach(change => {
                ...
                if (change.type === 'removed') {
                    vehicleList = vehicleList.filter(vehicle => vehicle.id !== idData)
                }
            })
            this.setState({
                vehicles: vehicleList
